...
int main() {
    ...
    if (err != sl::ERROR_CODE::SUCCESS) {
        std::cout << "Gagal membuka kamera ZED: " << sl::toString(err) << std::endl;
        return 1;
    }
    ...
    if (fx <= 1e-6 || fy <= 1e-6) {
        std::cerr << "ERROR: Nilai tidak valid. Periksa parameter kalibrasi kamera!" << std::endl;
        return 1;
    }
    
    // Path to ORB-SLAM3 vocabulary and config files
    string vocab_file = "/home/ichan/Documents/lib/lib-ORB-SLAM3/ORB_SLAM3/Vocabulary/ORBvoc.txt";
    string config_file = "/home/ichan/Documents/stereo-vo-orbslam3/zed_config.yaml";
    ...
    if (zed.grab() != sl::ERROR_CODE::SUCCESS) {
        std::cout << "Gagal mengambil frame awal dari ZED" << std::endl;
        return 1;
    }
    ...
    if (cv_frame_left.empty() || cv_frame_right.empty()) {
        std::cerr << "Frame kosong! Gagal ambil image dari ZED." << std::endl;
        return 1;
    }
    ...
    if (prevFeatures.size() == 0) {
        std::cerr << "ERROR: Tidak ada fitur yang terdeteksi di frame pertama! "
                  << "Coba arahkan kamera ke area bertekstur/terang." << std::endl;
        return 1;
    }

    namedWindow("FRAME ", WINDOW_AUTOSIZE);
    resizeWindow("FRAME ", 480, 480);
